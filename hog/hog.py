...
def hefty_hogs(player_score, opponent_score):
    ...
    if opponent_score == 0:
        ...
        while opponent_score >0:
            lastdig = opponent_score%10
            opponent_score = opponent_score//10
            newscore = digit_fn(lastdig)(player_score)
    return (newscore%30)
...
def hog_pile(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    player_dig = player_score % 10
    opponent_dig = opponent_score% 10
    if player_dig == opponent_dig:
        return player_dig
    else:
        return 0
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    while rolls <= 10:
        result = averaged_dice(rolls, dice)
        if result > high:
            high = result
            good_roll = rolls
        rolls+=1
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    if hefty_hogs_strategy(score, opponent_score, threshold, num_rolls) == 0:
        if hog_pile(score + hefty_hogs(score, opponent_score), opponent_score) or hefty_hogs(score, opponent_score) >= threshold:
            return 0
        else:
            return num_rolls
    else:
